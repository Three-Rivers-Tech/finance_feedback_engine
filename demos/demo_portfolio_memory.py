...
def demo_memory_engine():
    ...
    # Configuration
    config = {
        "persistence": {"storage_path": "data/demo_memory"},
        "portfolio_memory": {
            "enabled": True,
            "max_memory_size": 100,
            "learning_rate": 0.1,
    ...
    print(
        f"{'Provider':<15} {'Recommended':>12} {'Current Win Rate':>18} {'Sample Size':>13}"
    )
    print("-" * 62)
    ...
    for provider, weight in recommendations["recommended_weights"].items():
        ...
        print(
            f"{provider:<15} {weight:>11.1%} " f"{win_rate:>17.1f}% {sample_size:>13}"
        )
    ...
    for provider, stats in recommendations["provider_stats"].items():
        ...
        if "high_confidence_winrate" in stats:
            print(
                f"  High Confidence (>70%): {stats['high_confidence_winrate']:.1f}% win rate"
            )
            print(
                f"  Med Confidence (40-70%): {stats['medium_confidence_winrate']:.1f}% win rate"
            )
            print(
                f"  Low Confidence (<40%): {stats['low_confidence_winrate']:.1f}% win rate"
            )
