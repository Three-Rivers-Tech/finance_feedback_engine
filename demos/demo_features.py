...
def demo_sentiment_analysis():
    ...
    try:
        ...
        print(
            f"  Sentiment Score: {sentiment.get('sentiment_score', 0):.3f} (range: -1 to +1)"
        )
...
def demo_dynamic_weight_adjustment():
    ...
    config = load_config()
    config["decision_engine"] = {"ai_provider": "ensemble", "decision_threshold": 0.7}
    config["ensemble"] = {
        "enabled_providers": ["local", "cli", "codex", "qwen"],
        "provider_weights": {"local": 0.40, "cli": 0.20, "codex": 0.20, "qwen": 0.20},
...
def demo_comprehensive_analysis():
    ...
    try:
        ...
        print(
            f"  ✓ Technical indicators: RSI {data.get('technical', {}).get('rsi', 0):.1f}"
        )
        print(
            f"  ✓ Candlestick analysis: {data.get('technical', {}).get('candlestick_pattern', 'N/A')}"
        )

        if "sentiment" in data:
            print(
                f"  ✓ News sentiment: {data['sentiment'].get('overall_sentiment', 'N/A')}"
            )
