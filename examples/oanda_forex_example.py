...
def main():
    ...
    if not hasattr(engine.trading_platform, "get_portfolio_breakdown"):
        ...
        try:
            ...
            if positions:
                ...
                print()
                print(
                    f"{'Instrument':<12} {'Type':<6} {'Units':<12} {'Unrealized P&L':<15}"
                )
                print("-" * 60)
                ...
                for pos in positions:
                    ...
                    print(
                        f"{instrument:<12} {pos_type:<6} {units:<12,.0f} {pl_sign}${pl:,.2f}"
                    )
            ...
            if holdings:
                ...
                print()
                print(
                    f"{'Currency':<10} {'Net Amount':<15} {'USD Value':<15} {'Allocation %':<12}"
                )
                print("-" * 60)
                ...
                for holding in holdings:
                    ...
                    print(
                        f"{currency:<10} {amount:<15,.0f} ${value:<14,.2f} {alloc:<11.2f}%"
                    )
    ...
    try:
        decision = engine.analyze_asset(
            asset_pair="EUR_USD", include_sentiment=True, include_macro=True
        )
        ...
        print(
            f"Position Size:  {decision.get('recommended_position_size', 0):,.0f} units"
        )
        ...
        if decision.get("portfolio_context"):
            ...
            print(
                "   AI received current positions and exposure for context-aware decision"
            )
            print()
