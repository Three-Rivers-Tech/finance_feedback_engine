...
def demonstrate_position_sizing():
    ...
    print(
        f"Risk Amount: ${risk_dollars_aggressive:,.2f} (2% of ${account_balance:,.2f})"
    )
...
def demonstrate_long_position():
    ...
    for scenario_name, current_price in scenarios:
        ...
        print(
            f"  Status: {'PROFIT' if pnl['pnl_dollars'] > 0 else 'LOSS' if pnl['pnl_dollars'] < 0 else 'BREAK-EVEN'}"
        )
...
def demonstrate_comparison():
    ...
    for current_price in price_scenarios:
        ...
        print(
            f"  LONG Position:  ${long_pnl['pnl_dollars']:+8,.2f} ({long_pnl['pnl_percentage']:+6.2f}%)"
        )
        print(
            f"  SHORT Position: ${short_pnl['pnl_dollars']:+8,.2f} ({short_pnl['pnl_percentage']:+6.2f}%)"
        )
