...
def generate_dataset(decisions):
    ...
    for decision in decisions:
        ...
        if (
            decision.get("ai_provider") == "ensemble"
            and "ensemble_metadata" in decision
        ):
            ...
            meta_features = manager._generate_meta_features(
...
def run_training():
    ...
    if len(X) < 10:
        logging.warning(
            f"Not enough data to train model (found {len(X)} samples). Need at least 10."
        )
        return
