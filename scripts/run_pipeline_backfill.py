...
def parse_args():
    ...
    date_group.add_argument("--days", type=int, help="Backfill last N days from today")
    date_group.add_argument(
        "--start-date", type=str, help="Start date (YYYY-MM-DD). Requires --end-date."
    )
    ...
    # Storage options
    parser.add_argument(
        "--storage-path", type=str, help="Delta Lake storage path (overrides config)"
    )
...
async def main():
    ...
    if args.assets:
        asset_pairs = [
            standardize_asset_pair(ap.strip()) for ap in args.assets.split(",")
        ]
    else:
    ...
    storage_path = args.storage_path or config.get("delta_lake", {}).get(
        "storage_path", "data/delta_lake"
    )
    ...
    logger.info(
        f"Total Jobs:   {len(asset_pairs)} assets x {len(timeframes)} timeframes = {len(asset_pairs) * len(timeframes)}"
    )
    logger.info("=" * 80)
    ...
    try:
        ...
        logger.info(
            f"Throughput:      {result['total_records'] / elapsed:.2f} records/sec"
        )
        logger.info("=" * 80)
        ...
        for res in result["results"]:
            ...
            if status == "success":
                ...
                logger.error(
                    f"  ✗ {asset:10s} {tf:4s} → Error: {res.get('error', 'unknown')}"
                )
            else:
