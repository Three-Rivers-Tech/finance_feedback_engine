...
class THR236TestRunner:
    ...
    async def execute_rapid_trades(self):
        ...
        logger.info(
            f"Executed: {self.results['successful_executions']}/{self.num_trades}"
        )
    ...
    async def verify_outcomes(self, wait_time: int = 60):
        ...
        self.results["outcomes_missing"] = (
            self.results["successful_executions"] - total_outcomes
        )

        logger.info(
            f"Outcomes recorded: {total_outcomes}/{self.results['successful_executions']}"
        )
        ...
        if (
            self.results["outcomes_missing"] == 0
            and self.results["successful_executions"] > 0
        ):
