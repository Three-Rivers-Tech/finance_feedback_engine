...
def test_analyze_command_cleanup():
    ...
    with patch(
        "finance_feedback_engine.core.FinanceFeedbackEngine.analyze_asset"
    ) as mock_analyze:
    ...
    print(
        f"{'✅' if warning_count == 0 else '❌'} Analyze command: {warning_count} session warnings"
    )
...
def test_balance_command_cleanup():
    ...
    with patch(
        "finance_feedback_engine.core.FinanceFeedbackEngine.get_balance"
    ) as mock_balance:
        mock_balance.return_value = {"USD": 10000.0, "BTC": 0.5}
    ...
    print(
        f"{'✅' if warning_count == 0 else '❌'} Balance command: {warning_count} session warnings"
    )
    return warning_count == 0
...
def test_multiple_sequential_commands():
    ...
    print(
        f"{'✅' if warning_count == 0 else '❌'} Sequential commands: {warning_count} session warnings"
    )
    return warning_count == 0
