...
def test_granularity_mapping():
    ...
    assert (
        provider.GRANULARITIES["FOUR_HOUR"] == 21600
    ), "FOUR_HOUR should map to 21600s"
    assert provider.GRANULARITIES["SIX_HOUR"] == 21600, "SIX_HOUR should map to 21600s"

    # Test enum mapping
    assert (
        provider.GRANULARITY_ENUMS["4h"] == "SIX_HOUR"
    ), "4h enum should map to SIX_HOUR"
    assert (
        provider.GRANULARITY_ENUMS["6h"] == "SIX_HOUR"
    ), "6h enum should map to SIX_HOUR"
    assert (
        provider.GRANULARITY_ENUMS["FOUR_HOUR"] == "SIX_HOUR"
    ), "FOUR_HOUR enum should map to SIX_HOUR"
    assert (
        provider.GRANULARITY_ENUMS["SIX_HOUR"] == "SIX_HOUR"
    ), "SIX_HOUR enum should map to SIX_HOUR"
