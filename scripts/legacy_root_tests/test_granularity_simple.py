...
# Read the file
with open('/Users/cmp6510/.openclaw/workspace/ffe-local/finance_feedback_engine/data_providers/coinbase_data.py', 'r') as f:
    content = f.read()
...
if granularities_match:
    ...
    if '"4h": 21600' in granularities_text:
        ...
        print(f"   Found: {[line.strip() for line in granularities_text.split('\\n') if '4h' in line]}")
...
if enums_match:
    ...
    if '"4h": "SIX_HOUR"' in enums_text:
        ...
        print(f"   Found: {[line.strip() for line in enums_text.split('\\n') if '4h' in line]}")
...
print("3. Rebuild: cd /root/finance_feedback_engine && docker build -t finance-feedback-engine:latest .")
