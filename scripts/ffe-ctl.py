...
class FFEClient:
    ...
    def _request(self, method: str, endpoint: str, **kwargs) -> Dict[str, Any]:
        ...
        try:
            ...
            console.print(f"  Make sure the API server is running: uvicorn finance_feedback_engine.api.app:app")
            sys.exit(1)
...
def emergency_stop(ctx, close_positions):
    ...
    result = client.post(
        '/api/v1/bot/emergency-stop',
        params={'close_positions': close_positions}
    )
...
@click.option('--position-size', type=float, help='Position size percentage (0.001-0.05)')
