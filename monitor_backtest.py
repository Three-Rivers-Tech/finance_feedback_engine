...
def monitor_backtest_progress():
    ...
    while True:
        try:
            ...
            # Check if backtest log exists
            log_file = Path("/tmp/q1_backtest.log")
            ...
            if len(outcome_files) != last_outcome_count:
                ...
                print(f"| Memory: {vectors_file.stat().st_size / 1024 / 1024:.2f} MB " if vectors_file.exists() else "| Memory: 0 MB ", end="", flush=True)
