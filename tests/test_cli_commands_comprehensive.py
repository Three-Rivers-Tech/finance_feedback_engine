    ...
    with patch("finance_feedback_engine.cli.main.load_tiered_config", return_value={}):
        with patch(
            ...
            result2 = runner.invoke(
                cli, ["analyze", "btc-usd", "--provider", "ensemble"]
            )
            ...
            assert (
                passed_config.get("decision_engine", {}).get("ai_provider")
                == "ensemble"
            )
            MockEngine.return_value.generate_decision.assert_called_with("BTCUSD")
...
def test_dashboard_command_with_mocked_aggregator_and_display():
    ...
    with patch("finance_feedback_engine.cli.main.load_tiered_config", return_value={}):
        with patch(
            ...
            with patch(
                "finance_feedback_engine.cli.main.PortfolioDashboardAggregator",
                FakeAggregator,
            ):
                with patch(
                    "finance_feedback_engine.cli.main.display_portfolio_dashboard",
                    fake_display,
                ):
                    result = runner.invoke(cli, ["dashboard"])
                    assert result.exit_code == 0, result.output
                    assert display_called["called"] is True
                    assert display_called["data"] == {
...
def test_approve_command_yes_and_no():
    ...
    fake_engine = MagicMock()
    fake_engine.execute_decision.return_value = {
        "success": True,
        "message": "ok",
        "platform": "mock",
    }
    ...
    with runner.isolated_filesystem():
        ...
        with open(
            f"data/decisions/2025-12-12_{decision_id}.json", "w", encoding="utf-8"
        ) as f:
            json.dump(
                {
...
def test_portfolio_backtest_command():
    ...
    class FakePortfolioBacktester:
        ...
        def run_backtest(self, **kwargs):
            return {
                "portfolio_metrics": {"final_balance": 11000.0},
                "asset_results": {},
            }
...
def test_learning_report_command():
    ...
    fake_memory.generate_learning_validation_metrics.return_value = {
        ...
        "sample_efficiency": {
            ...
            "trades_to_60pct_win_rate": 50,
            "learning_speed_per_100_trades": 0.15,
        },
        "cumulative_regret": {
            "cumulative_regret": 10.0,
            "optimal_provider": "ensemble",
            "optimal_avg_pnl": 5.0,
            "avg_regret_per_trade": 0.1,
        },
        "concept_drift": {
            "drift_severity": "LOW",
            "drift_score": 0.02,
            "window_win_rates": [0.6, 0.62],
        },
        "thompson_sampling": {
            "exploration_rate": 0.2,
            "exploitation_convergence": 0.8,
            "dominant_provider": "local",
    ...
    with patch("finance_feedback_engine.cli.main.load_tiered_config", return_value={}):
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "finance_feedback_engine.cli.main.console.input", return_value=""
                ):  # Skip config editor
...
def test_install_deps_command():
    ...
    with patch("finance_feedback_engine.cli.main.load_tiered_config", return_value={}):
        with patch(
            "finance_feedback_engine.cli.main._check_dependencies",
            return_value=([], []),
        ):
