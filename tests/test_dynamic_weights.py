...
async def test_dynamic_weights_all_providers_respond(manager):
    ...
    assert result['ensemble_metadata']['adjusted_weights'] == config['ensemble']['provider_weights']
...
async def test_dynamic_weights_one_provider_fails(manager):
    ...
    result = await manager.aggregate_decisions(partial_decisions, failed_providers=failed)
...
async def test_dynamic_weights_multiple_providers_fail(manager):
    ...
    result = await manager.aggregate_decisions(minimal_decisions, failed_providers=failed_multiple)
...
async def test_dynamic_weights_only_one_provider_responds(manager):
    ...
    result = await manager.aggregate_decisions(single_decision, failed_providers=failed_most)
