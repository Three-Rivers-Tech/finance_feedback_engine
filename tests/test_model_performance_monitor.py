...
from finance_feedback_engine.monitoring.model_performance_monitor import (
...
class TestModelPerformanceMonitor:
    ...
    def test_record_prediction(self, monitor):
        ...
        monitor.record_prediction(
            features=features_df, prediction=prediction, timestamp=timestamp
        )
    ...
    def test_record_actual_outcome(self, monitor):
        ...
        monitor.record_prediction(
            features=features_df, prediction=prediction, timestamp=datetime.now()
        )
        ...
        pred_record = next(
            (p for p in monitor.predictions if p["prediction_id"] == "test_pred_456"),
            None,
        )
    ...
    def test_evaluate_performance(self, monitor):
        ...
        features_df = pd.DataFrame({"feature1": [1.0], "feature2": [2.0]})
        prediction = {"prediction_id": "test_eval_1", "action": "BUY", "confidence": 85}
        monitor.record_prediction(features_df, prediction, datetime.now())
        monitor.record_actual_outcome(
            "test_eval_1", {"success": True, "profit": 10.0}, datetime.now()
        )
    ...
    def test_detect_data_drift(self, monitor):
        ...
        price_drift = next(
            (f for f in drift_result["drifting_features"] if f["feature"] == "price"),
            None,
        )
        assert price_drift is not None
        # Either PSI or KS test should be in the reason due to the significant difference
        assert any(
            test in price_drift["reason"] for test in ["PSI:", "KS:", "p-value:"]
        )
...
class TestPerformanceMetrics:
    ...
    def test_multiple_predictions(self, monitor):
        ...
        assert performance["accuracy"] == pytest.approx(
            1.0, abs=1e-6
        )  # All recorded outcomes are successful
        assert performance["win_rate"] == pytest.approx(1.0, abs=1e-6)
