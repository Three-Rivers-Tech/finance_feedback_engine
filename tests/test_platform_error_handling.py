...
class TestCoinbaseConnectionErrors:
    ...
    def test_client_initialization_import_error(self, coinbase_platform):
        ...
        with patch('builtins.__import__', side_effect=ImportError("No module named 'coinbase'")):
            with pytest.raises(ValueError, match="Coinbase Advanced library not available"):
    ...
    def test_client_initialization_generic_error(self, coinbase_platform):
        ...
        with patch('coinbase.rest.RESTClient', side_effect=Exception("Connection refused")):
    ...
    def test_get_balance_network_timeout(self, coinbase_platform, mock_coinbase_client):
        ...
        mock_coinbase_client.get_futures_balance_summary.side_effect = Timeout("Request timeout")
    ...
    def test_get_balance_connection_error(self, coinbase_platform, mock_coinbase_client):
        """Should log warning and return empty dict on connection error."""
        coinbase_platform._client = mock_coinbase_client
        mock_coinbase_client.get_futures_balance_summary.side_effect = ConnectionError("Network unreachable")
        mock_coinbase_client.get_accounts.side_effect = ConnectionError("Network unreachable")
...
class TestCoinbaseResponseErrors:
    ...
    def test_get_balance_missing_futures_balance_summary(self, coinbase_platform, mock_coinbase_client):
    ...
    def test_get_balance_futures_error_but_spot_succeeds(self, coinbase_platform, mock_coinbase_client):
        ...
        mock_coinbase_client.get_futures_balance_summary.side_effect = Exception("Futures API error")
        ...
        type(usd_account).available_balance = PropertyMock(return_value=available_balance)
    ...
    def test_get_balance_spot_error_but_futures_succeeds(self, coinbase_platform, mock_coinbase_client):
        ...
        futures_response = MagicMock()
        balance_summary = {
            'total_usd_balance': {'value': '5000.0'}
        }
        type(futures_response).balance_summary = PropertyMock(return_value=balance_summary)
        mock_coinbase_client.get_futures_balance_summary.return_value = futures_response
...
class TestCoinbasePortfolioBreakdown:
    ...
    def test_get_portfolio_breakdown_futures_error_continues_with_spot(self, coinbase_platform, mock_coinbase_client):
        ...
        # Futures fails
        mock_coinbase_client.get_futures_balance_summary.side_effect = Exception("Futures error")
