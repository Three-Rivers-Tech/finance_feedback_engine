...
class TestTelegramBot:
    ...
    def test_format_decision_message(
        self, mock_bot_class, mock_tunnel, telegram_config, sample_decision
    ):
    ...
    def test_create_approval_keyboard(
        self, mock_bot_class, mock_tunnel, telegram_config
    ):
    ...
    async def test_send_approval_request(
        ...
        with patch.object(
            TelegramApprovalBot, "create_approval_keyboard", return_value=MagicMock()
        ):
            bot = TelegramApprovalBot(telegram_config)
    ...
    @patch("finance_feedback_engine.integrations.tunnel_manager.TunnelManager")
    @patch("telegram.Bot")
    async def test_send_approval_request_without_decision_id(
        self, mock_bot_class, mock_tunnel, telegram_config
    ):
    ...
    async def test_process_update_callback_query(
        self, mock_update_class, mock_bot_class, mock_tunnel, telegram_config
    ):
        ...
        with patch.object(
            bot, "_approve_decision", new_callable=AsyncMock
        ) as mock_approve:
    ...
    @patch("telegram.Bot")
    @patch("telegram.Update")
    async def test_process_update_unauthorized_user(
        self, mock_update_class, mock_bot_class, mock_tunnel, telegram_config
    ):
    ...
    @patch("telegram.Bot")
    @patch("telegram.Update")
    async def test_handle_message_start_command(
        self, mock_update_class, mock_bot_class, mock_tunnel, telegram_config
    ):
    ...
    @patch("finance_feedback_engine.integrations.tunnel_manager.TunnelManager")
    @patch("telegram.Bot")
    async def test_approve_decision_writes_file(
        self, mock_bot_class, mock_tunnel, telegram_config, tmp_path
    ):
        ...
        with patch(
            "finance_feedback_engine.integrations.telegram_bot.aiofiles.open"
        ) as mock_open:
            mock_file = AsyncMock()
    ...
    @patch("finance_feedback_engine.integrations.tunnel_manager.TunnelManager")
    @patch("telegram.Bot")
    async def test_reject_decision_writes_file(
        self, mock_bot_class, mock_tunnel, telegram_config
    ):
    ...
    def test_sanitize_decision_id(self, mock_bot_class, mock_tunnel, telegram_config):
        ...
        assert (
            bot._sanitize_decision_id("test/../../etc/passwd") == "test______etc_passwd"
        )
...
class TestTelegramBotWithoutInstallation:
    ...
    @patch("finance_feedback_engine.integrations.tunnel_manager.TunnelManager")
    async def test_send_message_without_bot(
        self, mock_tunnel, telegram_config, sample_decision
    ):
