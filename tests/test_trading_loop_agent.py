...
@pytest.mark.asyncio
async def test_execution_state_skips_pending_tracking_for_terminal_mock_fill(trading_agent, mock_dependencies):
    decision = {
        "id": "decision-mock-fill",
        "action": "ADD_SMALL_LONG",
    ...
    trading_agent.engine.execute_decision_async = AsyncMock(return_value={
        "success": True,
        "platform": "mock",
        "order_id": "mock-filled-123",
        "order_status": "FILLED",
    })

    await trading_agent.handle_execution_state()

    trading_agent.engine.order_status_worker.add_pending_order.assert_not_called()
    assert decision["execution_result"]["order_id"] == "mock-filled-123"
    assert decision["execution_status"] == "executed"
    assert decision["executed"] is True
    assert trading_agent.daily_trade_count == 1
