from finance_feedback_engine.config.provider_credentials import (
    resolve_provider_credentials,
    resolve_runtime_contract,
)


def test_resolve_provider_credentials_prefers_nested_and_platform_fallbacks() -> None:
    config = {
        "providers": {"coinbase": {"credentials": {"api_key": "key-1"}}},
        "platforms": [{"name": "oanda", "credentials": {"account_id": "abc"}}],
    }
...
def test_resolve_provider_credentials_includes_paper_platform_credentials() -> None:
    config = {
        "platforms": [{"name": "paper", "credentials": {"initial_cash_usd": 250000}}],
    }
...
def test_resolve_runtime_contract_marks_enabled_paper_runtime() -> None:
    runtime = resolve_runtime_contract(
        {"enabled_platforms": ["paper"], "agent": {"asset_pairs": ["BTCUSD"]}}
    )
...
def test_resolve_runtime_contract_uses_paper_flags_for_mixed_runtime() -> None:
    runtime = resolve_runtime_contract(
        {
