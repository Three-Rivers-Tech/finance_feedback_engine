...
class TestEnsembleProviderFailureTracking:
    ...
    async def test_ensemble_tracks_local_exception_as_failure(self):
        ...
        with patch.object(engine, '_local_ai_inference', side_effect=mock_local_raise):
            with patch.object(engine, '_cli_ai_inference', side_effect=mock_cli_success):
        ...
        assert 'local' in decision['ensemble_metadata']['providers_failed'], \
            f"Expected 'local' in providers_failed, got: {decision['ensemble_metadata']['providers_failed']}"
        assert 'cli' in decision['ensemble_metadata']['providers_used'], \
    ...
    async def test_ensemble_tracks_multiple_provider_failures(self):
        ...
        with patch.object(engine, '_local_ai_inference', side_effect=mock_raise):
            with patch.object(engine, '_cli_ai_inference', side_effect=mock_raise):
                with patch.object(engine, '_gemini_ai_inference', side_effect=mock_gemini_success):
                    decision = await engine._ensemble_ai_inference("test prompt")

        # Verify both local and cli are in providers_failed
        metadata = decision['ensemble_metadata']
        assert 'local' in metadata['providers_failed'], "Expected 'local' in providers_failed"
        assert 'cli' in metadata['providers_failed'], "Expected 'cli' in providers_failed"
        assert 'gemini' in metadata['providers_used'], "Expected 'gemini' in providers_used"
        assert metadata['num_active'] == 1, f"Expected 1 active provider, got {metadata['num_active']}"
        assert len(metadata['providers_failed']) == 2, f"Expected 2 failed providers, got {len(metadata['providers_failed'])}"
...
class TestSingleProviderFallbackBehavior:
    ...
    async def test_single_local_provider_returns_fallback_on_import_error(self):
        ...
        with patch('finance_feedback_engine.decision_engine.local_llm_provider.LocalLLMProvider') as mock_provider:
        ...
        assert decision['action'] == 'HOLD', f"Expected HOLD action, got {decision['action']}"
        assert 0 <= decision['confidence'] <= 100, f"Invalid confidence: {decision['confidence']}"
        assert 'fallback' in decision['reasoning'].lower() or 'import error' in decision['reasoning'].lower(), \
    ...
    async def test_single_local_provider_returns_fallback_on_runtime_error(self):
        ...
        # Should return fallback, NOT raise exception
        assert decision['action'] == 'HOLD', f"Expected HOLD action, got {decision['action']}"
        assert 'runtime error' in decision['reasoning'].lower() or 'fallback' in decision['reasoning'].lower(), \
    ...
    async def test_single_local_provider_returns_fallback_on_generic_exception(self):
        ...
        # Should return fallback, NOT raise exception
        assert decision['action'] == 'HOLD', f"Expected HOLD action, got {decision['action']}"
        assert 'unexpected error' in decision['reasoning'].lower() or 'fallback' in decision['reasoning'].lower(), \
...
class TestLocalPriorityFallbackChain:
    ...
    async def test_local_priority_soft_falls_back_to_remote(self):
        ...
        metadata = decision['ensemble_metadata']
        assert 'gemini' in metadata['providers_used'], \
...
class TestProviderWeightAdjustment:
    ...
    async def test_weights_adjust_when_provider_fails(self):
        ...
        assert 'local' not in adjusted_weights, \
        ...
        assert 'gemini' in adjusted_weights, "Gemini should have weight in adjusted_weights"
...
class TestEnsembleFallbackTiers:
    ...
    @pytest.mark.asyncio
    async def test_weighted_voting_falls_back_to_majority_on_insufficient_providers(self):
