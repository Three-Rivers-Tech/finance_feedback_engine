...
def mock_unified_platform(mock_coinbase_platform, mock_oanda_platform):
    ...
    unified.get_active_positions.side_effect = lambda: {
        "positions": [
            {**p, "platform": "coinbase"}
            for p in mock_coinbase_platform.get_active_positions()["positions"]
        ]
        + [
            {**p, "platform": "oanda"}
            for p in mock_oanda_platform.get_active_positions()["positions"]
        ]
    }
    return unified
...
def test_positions_cli_command_platform_error(mock_engine_cls, mock_coinbase_platform):
    ...
    mock_coinbase_platform.get_active_positions.side_effect = Exception(
