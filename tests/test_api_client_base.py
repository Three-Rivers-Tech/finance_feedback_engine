...
class TestAPIClientConcepts:
    ...
    async def test_async_post_request(self):
        ...
        response = await client._send_request_async(
            "POST", "/test_endpoint", json_data=post_data
        )
...
class TestRetryLogic:
    ...
    @pytest.mark.skip(
        reason="Test incorrectly patches the method being tested - needs refactoring"
    )
    @pytest.mark.asyncio
    async def test_retry_on_failure(self):
        ...
        mock_send_request_async = AsyncMock(
            side_effect=[
    ...
    async def test_exponential_backoff_concept(self):
        ...
        with patch.object(client, "_send_request_async", new=mock_send_request_async):
            response = await client._send_request_async(
                "GET", "/test", retries=3, backoff_factor=0.1
            )
            assert response == {"status": "success"}
...
class TestAuthentication:
    ...
    @pytest.mark.skip(
        reason="MockAPIClient doesn't capture request args - test needs refactoring"
    )
    @pytest.mark.asyncio
    async def test_bearer_token(self):
        ...
        assert (
            client.captured_request_args["headers"]["Authorization"]
            == f"Bearer {token}"
        )
