...
class TestAssetTypeValidation:
    ...
    def test_canonical_crypto_type(self, base_config, mock_query_function):
        ...
        with patch('finance_feedback_engine.decision_engine.provider_tiers.get_free_providers', return_value=['p1', 'p2', 'p3']):
            with patch.object(manager, '_is_valid_provider_response', return_value=True):
                ...
                try:
                    ...
                    pytest.fail(f"Canonical 'crypto' type should not raise ValueError: {e}")
        ...
        with patch('finance_feedback_engine.decision_engine.provider_tiers.get_free_providers', return_value=['p1', 'p2', 'p3']):
            with patch.object(manager, '_is_valid_provider_response', return_value=True):
                try:
                    ...
                    pytest.fail(f"Canonical 'forex' type should not raise ValueError: {e}")
        ...
        with patch('finance_feedback_engine.decision_engine.provider_tiers.get_free_providers', return_value=['p1', 'p2', 'p3']):
            with patch.object(manager, '_is_valid_provider_response', return_value=True):
                try:
                    ...
                    pytest.fail(f"Canonical 'stock' type should not raise ValueError: {e}")

    def test_variation_cryptocurrency_normalized_to_crypto(self, base_config, mock_query_function):
    ...
    def test_variation_equity_normalized_to_stock(self, base_config, mock_query_function):
        ...
        with patch('finance_feedback_engine.decision_engine.provider_tiers.get_free_providers', return_value=['p1', 'p2', 'p3']):
            with patch.object(manager, '_is_valid_provider_response', return_value=True):
                try:
                    ...
                    pytest.fail(f"Type with whitespace should normalize to 'crypto': {e}")
