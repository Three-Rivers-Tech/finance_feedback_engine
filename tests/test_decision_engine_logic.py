...
class TestMarketAnalysisHelpers:
    ...
    def test_calculate_price_change_positive(self, decision_engine):
        ...
        price_change = decision_engine.market_analyzer._calculate_price_change(
            market_data
        )
...
class TestPromptCreation:
    ...
    def test_create_ai_prompt_basic(
    ...
    def test_create_ai_prompt_with_portfolio(
        self, decision_engine, sample_market_data, sample_balance, sample_portfolio
    ):
    ...
    def test_create_ai_prompt_with_sentiment(
        self, decision_engine, sample_market_data, sample_balance
    ):
...
class TestDecisionGeneration:
    ...
    @pytest.mark.asyncio
    async def test_generate_decision_basic(
        self, decision_engine, sample_market_data, sample_balance
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_decision_with_portfolio(
        self, decision_engine, sample_market_data, sample_balance, sample_portfolio
    ):
    ...
    @pytest.mark.asyncio
    async def test_generate_decision_invalid_action_fallback(
        self, decision_engine, sample_market_data, sample_balance
    ):
...
class TestEnsembleIntegration:
    ...
    async def test_ensemble_decision_mocked(
        ...
        with patch.object(
            ...
            with patch.object(
                ensemble_engine, "_query_single_provider", new_callable=AsyncMock
            ) as mock_query:
                mock_query.return_value = {
                    "action": "BUY",
                    "confidence": 75,
...
class TestMarketRegimeDetection:
    ...
    @pytest.mark.asyncio
    async def test_detect_market_regime_success(
        self, decision_engine, mock_data_provider
    ):
    ...
    @pytest.mark.asyncio
    async def test_detect_market_regime_insufficient_data(
        self, decision_engine, mock_data_provider
    ):
...
class TestConfiguration:
    ...
    def test_config_flat_structure_backward_compatibility(self):
        ...
        config = {"ai_provider": "cli", "decision_threshold": 0.7, "local_models": []}
        engine = DecisionEngine(config)
...
class TestMonitoringContext:
    ...
    @pytest.mark.asyncio
    async def test_generate_decision_with_live_monitoring(
        self, decision_engine, sample_market_data, sample_balance
    ):
...
class TestBacktestMode:
    ...
    @pytest.mark.asyncio
    async def test_backtest_mode_generates_warning(
        self, test_config, sample_market_data, sample_balance
    ):
        ...
        with patch.object(engine, "_query_ai", new_callable=AsyncMock) as mock_query:
            ...
            with patch(
                "finance_feedback_engine.decision_engine.engine.logger"
            ) as mock_logger:
