...
class TestHistoricalDataIntegration:
    ...
    def test_historical_data_loop_closed(self, tmp_path):
        ...
        provider = HistoricalDataProvider(
            api_key="test", cache_dir=str(tmp_path / "cache")
        )
...
class TestSystemClosedLoops:
    ...
    def test_all_critical_todos_resolved(self):
        ...
        with open(telegram_file) as f:
            ...
            assert (
                len(active_todos) == 0
            ), f"Active TODOs found in telegram_bot.py: {active_todos}"
        ...
        with open(historical_file) as f:
            ...
            assert (
                len(active_todos) == 0
            ), f"Active TODOs found in historical_data_provider.py: {active_todos}"
    ...
    def test_validator_integration(self):
        ...
        assert isinstance(
            provider.validator, FinancialDataValidator
        ), "Validator not properly integrated"

        # Test validator is functional
        test_data = pd.DataFrame({"price": [100.0, 200.0], "volume": [1000, 2000]})
    ...
    def test_timeseries_store_integration(self):
        ...
        assert isinstance(
            provider.data_store, TimeSeriesDataStore
        ), "Data store not properly integrated"

        # Verify DataFrame methods exist
        assert hasattr(
            provider.data_store, "save_dataframe"
        ), "save_dataframe method missing"
        assert hasattr(
            provider.data_store, "load_dataframe"
