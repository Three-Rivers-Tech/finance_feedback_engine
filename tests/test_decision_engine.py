...
class TestDecisionEngineValidation:
    ...
    def test_empty_reasoning_handled_gracefully(self, decision_engine):
        ...
        with patch.object(
            decision_engine,
            "_query_ai",
            return_value={"action": "BUY", "confidence": 75, "reasoning": ""},
        ):
            decision = decision_engine.generate_decision(
                asset_pair="BTCUSD",
                market_data={
...
class TestDecisionEngineIntegration:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            (
                {"action": "SELL", "confidence": 80, "reasoning": "Bearish signals"},
                "SELL",
            ),
            (
                {"action": "HOLD", "confidence": 60, "reasoning": "Unclear market"},
                "HOLD",
            ),
        ],
    )
    def test_various_provider_responses(
        self, decision_engine, ai_response, expected_action
    ):
...
class TestDecisionEngineErrorHandling:
    ...
    def test_invalid_market_data_negative_prices(self, decision_engine):
        ...
        with patch.object(
            ...
            decision = decision_engine.generate_decision(
                asset_pair="BTCUSD", market_data=invalid_data, balance={"USD": 10000.0}
            )
