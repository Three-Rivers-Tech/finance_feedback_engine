...
class TestVotingStrategies:
    ...
    @pytest.mark.asyncio
    async def test_weighted_voting_consensus(self, ensemble_manager, sample_provider_decisions):
    ...
    async def test_majority_voting(self, ensemble_manager):
        ...
        decisions = {
            'local': {'action': 'BUY', 'confidence': 80, 'reasoning': 'Test1', 'amount': 0.05},
            'cli': {'action': 'BUY', 'confidence': 75, 'reasoning': 'Test2', 'amount': 0.05},
            'codex': {'action': 'SELL', 'confidence': 70, 'reasoning': 'Test3', 'amount': 0.03}
        }
    ...
    async def test_stacking_ensemble(self, ensemble_manager):
        ...
        decisions = {
            'local': {'action': 'BUY', 'confidence': 80, 'reasoning': 'Test1', 'amount': 0.05},
            'cli': {'action': 'BUY', 'confidence': 75, 'reasoning': 'Test2', 'amount': 0.05},
            'codex': {'action': 'HOLD', 'confidence': 60, 'reasoning': 'Test3', 'amount': 0.0}
        }
...
class TestFallbackTiers:
    ...
    @pytest.mark.asyncio
    async def test_primary_strategy_success(self, ensemble_manager, sample_provider_decisions):
    ...
    async def test_fallback_to_majority(self, ensemble_manager):
        ...
        with patch.object(ensemble_manager, '_weighted_voting', side_effect=ValueError("Weighted failed")):
            decisions = {
                'local': {'action': 'BUY', 'confidence': 80, 'reasoning': 'Test1', 'amount': 0.05},
                'cli': {'action': 'BUY', 'confidence': 75, 'reasoning': 'Test2', 'amount': 0.05}
            }
    ...
    async def test_fallback_to_average(self, ensemble_manager):
        ...
        with patch.object(ensemble_manager, '_weighted_voting', side_effect=ValueError("Weighted failed")):
            with patch.object(ensemble_manager, '_majority_voting', side_effect=ValueError("Majority failed")):
                decisions = {
                    'local': {'action': 'BUY', 'confidence': 80, 'reasoning': 'Test1', 'amount': 0.05},
                    'cli': {'action': 'SELL', 'confidence': 75, 'reasoning': 'Test2', 'amount': 0.03}
                }
    ...
    async def test_fallback_to_single_provider(self, ensemble_manager):
        ...
        with patch.object(ensemble_manager, '_weighted_voting', side_effect=ValueError("Failed")):
            with patch.object(ensemble_manager, '_majority_voting', side_effect=ValueError("Failed")):
                with patch.object(ensemble_manager, '_simple_average', side_effect=ValueError("Failed")):
                    decisions = {
                        'local': {'action': 'BUY', 'confidence': 80, 'reasoning': 'Test1', 'amount': 0.05}
                    }
...
class TestProviderFailures:
    ...
    async def test_partial_provider_failure(self, ensemble_manager):
        ...
        result = await ensemble_manager.aggregate_decisions(decisions, failed_providers=failed)
    ...
    async def test_confidence_adjustment_for_failures(self, ensemble_manager):
        ...
        decisions = {
            'local': {'action': 'BUY', 'confidence': 100, 'reasoning': 'Test', 'amount': 0.05}
        }
...
class TestDebateMode:
    ...
    def test_debate_mode_missing_provider_validation(self):
        ...
        with pytest.raises(ValueError, match="debate providers are not in enabled_providers"):
    ...
    def test_debate_decisions_synthesis(self, ensemble_manager):
        ...
        bull_case = {'action': 'BUY', 'confidence': 85, 'reasoning': 'Bullish case', 'amount': 0.05}
        bear_case = {'action': 'SELL', 'confidence': 75, 'reasoning': 'Bearish case', 'amount': 0.03}
        judge = {'action': 'BUY', 'confidence': 70, 'reasoning': 'Judge decision', 'amount': 0.04}
...
class TestQuorum:
    ...
    async def test_quorum_met(self, ensemble_manager):
        ...
        decisions = {
            'local': {'action': 'BUY', 'confidence': 80, 'reasoning': 'Test1', 'amount': 0.05},
            'qwen': {'action': 'BUY', 'confidence': 75, 'reasoning': 'Test2', 'amount': 0.05}
        }
    ...
    async def test_quorum_not_met_penalty(self, ensemble_manager):
        ...
        decisions = {
            'codex': {'action': 'BUY', 'confidence': 100, 'reasoning': 'Test', 'amount': 0.05}
        }
...
class TestMetaLearner:
    ...
    def test_meta_learner_fallback_params(self):
        ...
        assert len(manager.meta_learner.classes_) >= 2  # At least BUY, HOLD (may include SELL)
