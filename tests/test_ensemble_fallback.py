...
def print_decision_summary(result: dict, title: str):
    ...
    table.add_row(
        "Providers Failed",
        str(len(meta.get('providers_failed', [])))
    )
    table.add_row(
        "Failure Rate",
        f"{meta.get('failure_rate', 0):.1%}"
    )
    table.add_row("Fallback Tier", meta.get('fallback_tier', 'N/A'))
    table.add_row(
        "Agreement Score",
        f"{meta.get('agreement_score', 0):.2f}"
    )
    table.add_row(
        "Confidence Adjusted",
        str(meta.get('confidence_adjusted', False))
    )
    
    if meta.get('confidence_adjusted'):
        table.add_row(
            "Original Confidence",
            f"{meta.get('original_confidence', 0)}%"
        )
        table.add_row(
            "Adjustment Factor",
            f"{meta.get('confidence_adjustment_factor', 1.0):.3f}"
        )
...
def test_two_providers_fail():
    ...
    console.print(
        "\n[yellow]⚠ Weights renormalized: 0.25 → 0.50 for "
        "active providers[/yellow]"
    )
    console.print(
        "[yellow]⚠ Confidence degraded by 15% (2/4 active)[/yellow]"
    )
...
def test_three_providers_fail():
    ...
    console.print(
        "[yellow]⚠ Confidence degraded by 22.5% (1/4 active)[/yellow]"
    )
...
def run_all_tests():
    ...
    try:
        ...
        console.print()
        console.print(
            "[bold]See docs/ENSEMBLE_FALLBACK_SYSTEM.md for details[/bold]"
        )
