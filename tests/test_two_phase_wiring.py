...
class TestTwoPhaseEnsembleWiring:
    ...
    @patch(
        "finance_feedback_engine.decision_engine.engine.DecisionEngine._query_single_provider"
    )
    def test_two_phase_disabled_uses_standard_aggregation(self, mock_query):
        ...
        config = {
            "decision_engine": {"ai_provider": "ensemble", "local_models": []},
            "ensemble": {
        ...
        result = engine._ensemble_ai_inference(
            prompt="Test prompt", asset_pair="BTCUSD", market_data={"type": "crypto"}
        )
    ...
    @patch(
        "finance_feedback_engine.decision_engine.ensemble_manager.EnsembleDecisionManager.aggregate_decisions_two_phase"
    )
    @patch(
        "finance_feedback_engine.decision_engine.engine.DecisionEngine._query_single_provider"
    )
    def test_two_phase_enabled_calls_aggregate_decisions_two_phase(
        self, mock_query, mock_two_phase
    ):
