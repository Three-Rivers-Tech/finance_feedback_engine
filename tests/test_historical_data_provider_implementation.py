...
def mock_alpha_vantage():
    ...
    with patch('finance_feedback_engine.data_providers.historical_data_provider.AlphaVantageProvider') as mock:
        yield mock
...
class TestHistoricalDataProvider:
    ...
    @patch('finance_feedback_engine.data_providers.historical_data_provider.asyncio.run')
    def test_fetch_raw_data(self, mock_asyncio_run, mock_alpha_vantage, tmp_path, sample_ohlcv_data):
    ...
    @patch('finance_feedback_engine.data_providers.historical_data_provider.asyncio.run')
    def test_fetch_raw_data_caching(self, mock_asyncio_run, mock_alpha_vantage, tmp_path):
    ...
    def test_get_historical_data_uses_cache_first(self, tmp_path):
        ...
        with patch.object(provider.data_store, 'load_dataframe', return_value=cached_df) as mock_load:
            with patch.object(provider, '_fetch_raw_data') as mock_fetch:
                df = provider.get_historical_data(
                    'BTCUSD',
                    start_date='2024-01-01',
    ...
    def test_get_historical_data_validates_data(self, tmp_path):
        ...
        with patch.object(provider, '_fetch_raw_data') as mock_fetch:
            dates = pd.date_range('2024-01-01', periods=5, freq='1h', tz='UTC')
            mock_df = pd.DataFrame({
                'open': [100.0, -50.0, 105.0, 102.0, 108.0],  # Negative price (invalid)
            ...
            with patch.object(provider.validator, 'validate_dataframe') as mock_validate:
    ...
    def test_get_historical_data_handles_persistence_failure(self, tmp_path):
        ...
        with patch.object(provider, '_fetch_raw_data') as mock_fetch:
            ...
            with patch.object(provider.data_store, 'save_dataframe', side_effect=Exception("Disk full")):
...
class TestHistoricalDataProviderIntegration:
    ...
    def test_full_workflow_without_cache(self, tmp_path):
        ...
        with patch('finance_feedback_engine.data_providers.historical_data_provider.asyncio.run') as mock_run:
            # Mock API response
            candles = [
                {
