...
def test_verify_model_success():
    ...
    # Second call: ollama show --format json (with digest)
...
def test_verify_model_failure_missing_digest():
    ...
    # First call: ollama list --format json (returns empty)
    list_json = json.dumps([])
    # Second call: ollama show with missing digest
...
def test_download_model_calls_verify_success_progress_off():
    mi = ModelInstaller(data_dir="data")
    with patch("subprocess.run") as run:
        # First call: ollama pull
        pull_proc = _completed_proc(returncode=0)
        # Second call: ollama list --format json (for verification)
        list_json = json.dumps([])
        list_proc = _completed_proc(stdout=list_json, returncode=0)
        # Third call: ollama show --format json (for verification with digest)
        show_json = json.dumps({"digest": "sha256:abc123"})
        show_proc = _completed_proc(stdout=show_json, returncode=0)
        run.side_effect = [pull_proc, list_proc, show_proc]
