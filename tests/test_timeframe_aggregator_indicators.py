...
class TestRSICalculation:
    ...
    @pytest.mark.skip(
        reason="Synthetic sinusoidal data doesn't create realistic price changes for RSI"
    )
...
class TestMACDCalculation:
    ...
    def test_macd_uptrend_positive(self, aggregator, synthetic_uptrend_candles):
        ...
        assert (
            macd["histogram"] > 0
        ), f"Expected positive MACD histogram in uptrend, got {macd['histogram']}"
    ...
    def test_macd_downtrend_negative(self, aggregator, synthetic_downtrend_candles):
        ...
        assert (
            macd["histogram"] < 0
        ), f"Expected negative MACD histogram in downtrend, got {macd['histogram']}"
...
class TestBollingerBands:
    ...
    def test_bbands_middle_is_sma(self, aggregator, synthetic_ranging_candles):
        ...
        bbands = aggregator._calculate_bollinger_bands(
            synthetic_ranging_candles, period=20
        )
    ...
    def test_bbands_percent_b_range(self, aggregator, synthetic_uptrend_candles):
        ...
        assert (
            -50 <= bbands["percent_b"] <= 150
        ), f"Unexpected %B: {bbands['percent_b']}"
...
class TestADXCalculation:
    ...
    def test_adx_with_insufficient_data(self, aggregator):
        ...
        candles = [
            {"high": 100 + i, "low": 99 + i, "close": 99.5 + i} for i in range(20)
        ]
    ...
    def test_adx_downtrend_minus_di_higher(
        self, aggregator, synthetic_downtrend_candles
    ):
...
class TestATRCalculation:
    ...
    def test_atr_with_insufficient_data(self, aggregator):
        ...
        candles = [
            {"high": 100 + i, "low": 99 + i, "close": 99.5 + i} for i in range(10)
        ]
    ...
    def test_atr_higher_in_volatile_market(self, aggregator):
        ...
        calm_candles = [{"high": 100.1, "low": 99.9, "close": 100.0} for _ in range(50)]
...
class TestSignalStrength:
    ...
    def test_signal_strength_oversold(self, aggregator):
        ...
        assert (
            strength >= 70
        ), f"Expected high strength for strong signals, got {strength}"
...
class TestDetectTrendEnhanced:
    ...
    def test_detect_trend_uptrend_confirmation(
        self, aggregator, synthetic_uptrend_candles
    ):
    ...
    def test_detect_trend_all_fields_present(
        self, aggregator, synthetic_ranging_candles
    ):
