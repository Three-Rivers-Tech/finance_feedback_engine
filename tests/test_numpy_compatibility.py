...
class TestNumpyCompatibility:
    ...
    def test_numpy_version_requirement(self):
        ...
        assert numpy_version >= version.parse("2.2.0"), f"numpy {np.__version__} < 2.2.0"
        assert numpy_version < version.parse("2.4.0"), f"numpy {np.__version__} >= 2.4.0 (untested)"
    ...
    def test_version_compatibility_matrix(self):
        ...
        assert version.parse(pd.__version__) >= version.parse("2.2.3"), f"pandas {pd.__version__} < 2.2.3"
        assert version.parse(sklearn.__version__) >= version.parse("1.7.0"), f"scikit-learn {sklearn.__version__} < 1.7.0 (numpy 2.x support requires 1.7.0+)"
        assert version.parse(np.__version__) >= version.parse("2.2.0"), f"numpy {np.__version__} < 2.2.0"
