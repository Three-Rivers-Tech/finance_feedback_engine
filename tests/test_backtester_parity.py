...
def test_mock_platform_maker_vs_taker_slippage_and_fee():
    ...
    taker = platform.execute_trade({**common, "action": "BUY", "order_type": "market"})
    maker = platform.execute_trade({**common, "action": "BUY", "order_type": "limit"})

    assert taker["success"] and maker["success"]
    assert maker.get("fee_rate", 1) < taker.get("fee_rate", 0)
    assert maker.get("slippage_applied", 1) <= taker.get("slippage_applied", 0)
    assert maker.get("order_type") == "limit"
