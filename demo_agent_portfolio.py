...
def example_1_view_portfolio():
    ...
    try:
        ...
        # Display summary
        console.print(f"Total Value: [green]${portfolio['total_value_usd']:,.2f}[/green]")
...
def example_2_check_concentration():
    ...
    console.print("Agent Task: [italic]'Alert me if any asset exceeds 30% allocation'[/italic]\n")
...
def example_3_crypto_vs_fiat():
    ...
    try:
        ...
        crypto_value = sum(
            h['value_usd'] for h in portfolio['holdings']
            if h['asset'] not in FIAT
        )
        ...
        table.add_row(
            "Cryptocurrency",
            f"${crypto_value:,.2f}",
            f"{crypto_value/total*100:.1f}%"
        )
        table.add_row(
            "Fiat/Stablecoins",
            f"${fiat_value:,.2f}",
            f"{fiat_value/total*100:.1f}%"
        )
        table.add_row(
            "[bold]Total[/bold]",
...
def example_4_context_aware_decision():
    ...
    try:
        ...
        # Find BTC holding
        btc_holding = next(
            (h for h in portfolio['holdings'] if h['asset'] == 'BTC'),
            None
        )
        ...
        if btc_holding:
            ...
            if allocation > 25:
                ...
                color = "green"
                reason = f"BTC allocation ({allocation:.1f}%) is below typical 15-20% target"
            ...
            console.print("No BTC position found - consider initial allocation of 5-10%")
    except Exception as e:
        console.print(f"[yellow]{e}[/yellow]")
...
def main():
    ...
    console.print("[dim]Agent now has full portfolio awareness for context-aware recommendations[/dim]\n")
