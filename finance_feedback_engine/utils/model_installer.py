...
class ModelInstaller:
    ...
    def _verify_model(self, model: str) -> bool:
        ...
        try:
            ...
            if lines:
                ...
                if any(h in first_line for h in header_keywords) or (
                    "  " in first_line
                ):
                    header_found = True
            ...
            if show.returncode == 0:
                try:
                    ...
                    if "digest" in show_data and show_data["digest"]:
                        ...
                        logger.error(
                            f"Verification failed for {model}: missing digest in show output"
                        )
                        return False
                except json.JSONDecodeError:
                    logger.error(
                        f"Verification failed for {model}: invalid JSON from 'ollama show'"
                    )
                    return False

            logger.error(
                f"Verification failed for {model}: not listed by 'ollama list' and 'ollama show' failed ({show.returncode})"
            )
            return False
        except subprocess.TimeoutExpired:
    ...
    def _download_models_parallel(
        self, models_to_install: List[str], max_workers: int = 2
    ) -> Dict[str, bool]:
        """
    ...
    def ensure_models_installed(self, force_reinstall: bool = False) -> bool:
        ...
        print(
            f"Total download size: ~{sum(MODEL_DOWNLOAD_SIZES.get(m, 0) for m in models_to_install):.1f}GB"
        )
