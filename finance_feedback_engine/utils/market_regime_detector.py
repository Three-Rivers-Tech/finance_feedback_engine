...
class MarketRegimeDetector:
    ...
    def _calculate_directional_indicators(self, df: pd.DataFrame) -> tuple:
        ...
        plus_di = (
            pd.Series(plus_di, index=df.index).replace([np.inf, -np.inf], 0).fillna(0)
        )
        minus_di = (
            pd.Series(minus_di, index=df.index).replace([np.inf, -np.inf], 0).fillna(0)
        )
    ...
    def detect_regime(
        self, market_data: Union[Dict[str, Any], pd.DataFrame]
    ) -> str:  # noqa: E128
        """
