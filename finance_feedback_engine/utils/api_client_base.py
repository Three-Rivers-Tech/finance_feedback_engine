...
def log_before_retry(retry_state):
    """Log before retrying."""
    logger.info(
        f"Retrying {retry_state.fn.__name__} in {retry_state.next_action.sleep:.2f} seconds..."
    )
    ...
    def _send_request(
        ...
        def _send():
            try:
                ...
                logger.debug(
                    f"Sending {method} request to {url} with params={params}, data={data}, json={json_data}"
                )
                ...
                if 400 <= e.response.status_code < 500:
                    ...
                    logger.warning(
                        f"Server error ({e.response.status_code}) for {url}: {e.response.text}"
                    )
                    raise
                ...
                logger.error(
                    f"Failed to decode JSON response from {url}. Response: {e.doc}"
                )
                raise
