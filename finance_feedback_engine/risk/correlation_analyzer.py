...
class CorrelationAnalyzer:
    ...
    def __init__(self, lookback_days: int = 30):
        ...
        logger.info(
            "CorrelationAnalyzer initialized with %d-day lookback", lookback_days
        )
    ...
    def calculate_pearson_correlation(
        ...
        n = len(returns_a)
        if n < 10:
            logger.warning("Insufficient data for correlation (%d points, need 10+)", n)
            return None
        ...
        covariance = sum(
            (a - mean_a) * (b - mean_b) for a, b in zip(returns_a, returns_b)
        ) / (n - 1)
    ...
    def analyze_platform_correlations(
        ...
        highly_correlated = self.find_highly_correlated_pairs(
            correlation_matrix, self.correlation_threshold
        )

        # Determine max correlation
        max_correlation = 0.0
        if correlation_matrix:
            max_correlation = max(abs(corr) for corr in correlation_matrix.values())
    ...
    def analyze_cross_platform_correlation(
        ...
        if not coinbase_price_history or not oanda_price_history:
            return {"cross_correlations": [], "max_correlation": 0.0, "warning": None}
        ...
        if cross_correlations:
            max_correlation = max(abs(c["correlation"]) for c in cross_correlations)
