...
class PulseDataFetcher:
    ...
    def fetch_pulse(self, asset_pair: str) -> Optional[Dict[str, Any]]:
        ...
        if hasattr(self.engine, "monitoring_context_provider"):
            try:
                context = (
                    self.engine.monitoring_context_provider.get_monitoring_context(
                        asset_pair
                    )
                )
        ...
        if hasattr(self.engine, "data_provider"):
            try:
                fetched = self.engine.data_provider.get_comprehensive_market_data(
                    asset_pair
                )
...
class TimeframeTableFormatter:
    ...
    def format_timeframe(self, tf_data: TimeframeData) -> Table:
        ...
        table.add_row("MACD", f"{tf_data.macd.get('macd', 0):.2f}", macd_status)
        ...
        table.add_row("ADX", f"{tf_data.adx.get('adx', 0):.1f}", adx_status)
    ...
    def _get_trend_color(trend: str) -> str:
        ...
        return {"UPTREND": "green", "DOWNTREND": "red", "RANGING": "yellow"}.get(
            trend.upper(), "white"
        )
...
class PulseDisplayService:
    ...
    def _display_alignment(self, alignment: Dict[str, Any]):
        ...
        self.console.print(
            f"  [{alignment['color']}]{alignment['alignment']}[/{alignment['color']}]"
        )
        self.console.print(
