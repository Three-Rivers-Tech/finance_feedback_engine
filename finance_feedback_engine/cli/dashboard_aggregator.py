...
class DashboardDataAggregator:
        ...
        try:
            ...
            if config:
                ...
                try:
                    ...
                    current_pnl_pct = (
                        risk_metrics.get("unrealized_pnl_percent", 0.0) or 0.0
                    )
    ...
    def get_portfolio_snapshot(self) -> Dict[str, Any]:
        ...
        try:
            ...
            largest_position_pct = (
                position_concentration.get("largest_position_pct", 0.0) or 0.0
            )
            diversification_score = (
                position_concentration.get("diversification_score", 0.0) or 0.0
            )
    ...
    def _evaluate_risk_limits(self, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            drawdown_limit = (
                getattr(config, "portfolio_stop_loss_percentage", 0.15)
                if config
                else 0.15
            )
            drawdown_status = (
                "ok"
                if drawdown_pct < drawdown_limit * 0.8
                else "warning" if drawdown_pct < drawdown_limit else "critical"
            )
            ...
            var_pct = (
                (
                    risk_metrics.get("total_exposure_usd", 0.0)
                    / max(risk_metrics.get("account_value", 1.0), 1.0)
                )
                * 100
                if risk_metrics.get("account_value", 0.0) > 0
                else 0.0
            )
            var_limit = 500  # 500% (5x leverage limit)
            var_status = (
                "ok"
                if var_pct < var_limit * 0.8
                else "warning" if var_pct < var_limit else "critical"
            )
            ...
            concentration_status = (
                "ok"
                if largest_pos_pct < 0.4
                else "warning" if largest_pos_pct < 0.5 else "critical"
            )
            ...
            return {
                ...
                "var": {"status": var_status, "current": var_pct, "limit": var_limit},
                "concentration": {
                ...
                try:
                    ...
                    if entry_time:
                        if isinstance(entry_time, str):
                            ...
                            duration_hours = (
                                datetime.now(entry_dt.tzinfo) - entry_dt
                            ).total_seconds() / 3600
        ...
        try:
            ...
            for asset in watchlist:
                try:
                    ...
                    confluence_strength = (
                        trend_alignment.get("confluence_strength", 0.0) or 0.0
                    )
        ...
        try:
            ...
            if trades:
                sorted_trades = sorted(
                    trades, key=lambda t: t.get("realized_pnl", 0.0) or 0.0
                )
                if sorted_trades:
