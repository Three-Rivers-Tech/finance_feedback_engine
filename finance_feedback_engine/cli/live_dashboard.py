...
class LiveDashboard:
    ...
    def render(self) -> RenderableType:
        ...
        try:
            ...
            layout["portfolio"].update(self.portfolio_panel.render(self._portfolio_data))
            layout["trades"].update(self.active_trades_panel.render(self._active_trades))
            layout["market"].update(self.market_pulse_table.render(self._market_pulse))
            layout["decisions"].update(self.decision_log_panel.render(self._recent_decisions))
            layout["footer"].update(self.performance_panel.render(self._performance_stats))
...
class AgentStatusPanel:
    ...
    def render(self, status: Dict[str, Any]) -> RenderableType:
        ...
        if not status:
            return Panel("[dim]Initializing agent...[/dim]", title="Agent Status", border_style="cyan")
        ...
        trades_color = 'red' if daily_trades >= max_daily_trades else 'yellow' if daily_trades >= max_daily_trades * 0.8 else 'green'
        status_parts2.append(f"[{trades_color}]Daily Trades: {daily_trades}/{max_daily_trades}[/{trades_color}]")
        ...
        if kill_switch.get('active'):
            ...
            status_parts2.append(f"[{ks_color}]Kill Switch: {kill_switch_status} Active (Loss: {loss_pct:.0f}%, Gain: {gain_pct:.0f}%)[/{ks_color}]")
        else:
        ...
        return Panel(content, title="[bold cyan]AUTONOMOUS AGENT STATUS[/bold cyan]", border_style="cyan")
...
class PortfolioPanel:
    ...
    def render(self, data: Dict[str, Any]) -> RenderableType:
        ...
        if not data or data.get('balance', 0) == 0:
            return Panel("[dim]Waiting for portfolio data...[/dim]", title="Portfolio", border_style="blue")
        ...
        lines.append(f"[bold white]Balance: ${balance:,.2f} ({pnl_sign}{unrealized_pnl_pct:.2f}%)[/bold white]")
        lines.append(f"[{pnl_color}]Unrealized P&L: {pnl_sign}${unrealized_pnl:,.2f}[/{pnl_color}]")

        # Exposure and leverage
        exposure_pct = (total_exposure / balance * 100) if balance > 0 else 0
        lines.append(f"[white]Exposure: ${total_exposure:,.0f} ({exposure_pct:.0f}%)[/white]")

        leverage_color = 'red' if leverage > 3 else 'yellow' if leverage > 2 else 'green'
        ...
        lines.append(f" [{dd_color}]{dd_icon} Drawdown: {dd_current:.1f}% / {dd_limit * 100:.0f}%[/{dd_color}]")
        ...
        lines.append(f" [{var_color}]{var_icon} Exposure Ratio: {var_current:.0f}% / {var_limit:.0f}%[/{var_color}]")
        ...
        lines.append(f" [{conc_color}]{conc_icon} Concentration: {conc_status.upper()}[/{conc_color}]")

        content = "\n".join(lines)
        return Panel(content, title="[bold blue]PORTFOLIO[/bold blue]", border_style="blue")
...
class ActiveTradesPanel:
    ...
    def render(self, trades: List[Dict[str, Any]]) -> RenderableType:
        ...
        for i, trade in enumerate(trades[:2]):  # Max 2 concurrent trades
            ...
            lines.append(f"[bold {side_color}]{asset} {side}[/bold {side_color}] [dim]({duration_str})[/dim]")

            # Prices
            lines.append(f"[white]Entry: ${entry_price:,.2f}[/white]")

            price_change_pct = ((current_price - entry_price) / entry_price * 100) if entry_price > 0 else 0.0
            if side == 'SHORT':
                price_change_pct = -price_change_pct

            price_color = 'green' if price_change_pct >= 0 else 'red'
            price_sign = '+' if price_change_pct >= 0 else ''
            lines.append(f"[{price_color}]Current: ${current_price:,.2f} ({price_sign}{price_change_pct:.2f}%)[/{price_color}]")

            # P&L
            pnl_color = 'green' if pnl >= 0 else 'red'
            pnl_sign = '+' if pnl >= 0 else ''
            lines.append(f"[bold {pnl_color}]P&L: {pnl_sign}${pnl:,.2f} (Peak: ${peak_pnl:,.2f})[/bold {pnl_color}]")
        ...
        content = "\n".join(lines)
        return Panel(content, title=f"[bold green]ACTIVE TRADES ({len(trades)}/2)[/bold green]", border_style="green")
...
class MarketPulseTable:
    ...
    def render(self, pulse_data: List[Dict[str, Any]]) -> RenderableType:
        ...
        table = Table(title="Market Pulse", show_header=True, header_style="bold magenta", border_style="magenta")
        ...
        for item in pulse_data:
            ...
            change_str = f"[{change_color}]{change_sign}{change_1m_pct:.2f}%[/{change_color}]"
            ...
            table.add_row(asset, price_str, change_str, trend_formatted, confluence_str, signal_formatted)
...
class DecisionLogPanel:
    ...
    def render(self, decisions: List[Dict[str, Any]]) -> RenderableType:
        ...
        content = "\n".join(lines)
        return Panel(content, title="[bold yellow]RECENT DECISIONS[/bold yellow]", border_style="yellow")
...
class PerformancePanel:
    ...
    def render(self, stats: Dict[str, Any]) -> RenderableType:
        ...
        parts.append(f"[{pnl_color}]Total P&L: {pnl_sign}${total_pnl:,.0f}[/{pnl_color}]")
        ...
        if streak_type != 'NONE' and streak_count > 0:
            ...
            parts2.append(f"[{streak_color}]Streak: {streak_count}{streak_label}[/{streak_color}]")

        line2 = " │ ".join(parts2)

        content = f"{line1}\n{line2}"

        return Panel(content, title="[bold cyan]PERFORMANCE (24h)[/bold cyan]", border_style="cyan")
