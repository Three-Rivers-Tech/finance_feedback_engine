...
def analyze(ctx, asset_pair, provider, show_pulse):
    ...
    try:
        ...
        if decision.get("action") == "NO_DECISION":
            ...
            console.print(
                f"[yellow]Reason: {decision.get('reasoning', 'Unknown')}[/yellow]"
            )
            ...
            try:
                ...
                if fcntl:
                        ...
                        with open(log_path, "a", encoding="utf-8") as f:
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    console.print(
                                        f"[yellow]Warning: fsync failed: {e}[/yellow]"
                                    )
                            finally:
                                try:
                                    fcntl.flock(f.fileno(), fcntl.LOCK_UN)
                                except IOError as e:
                                    # Failed to unlock the file - log but continue
                                    console.print(
                                        f"[yellow]Warning: Failed to unlock file: {e}[/yellow]"
                                    )
                    except Exception as write_err:
                        console.print(
                            f"[yellow]Warning: Failed to append JSONL entry with lock: {write_err}[/yellow]"
                        )
                else:
                    ...
                    try:
                        ...
                        console.print(
                            f"[yellow]Warning: Failed to append JSONL entry: {append_err}[/yellow]"
                        )
        ...
        if "sentiment" in md and md["sentiment"].get("available"):
            ...
            console.print("\nNews Sentiment:")
            console.print(
                f"  Overall: " f"{sent.get('overall_sentiment', 'neutral').upper()}"
            )
        ...
        if decision.get("ensemble_metadata"):
            ...
            if meta.get("local_models_used"):
                console.print(
                    f"  Local Models Used: {', '.join(meta['local_models_used'])}"
                )
