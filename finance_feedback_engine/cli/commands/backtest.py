...
@click.option(
    "--initial-balance", type=float, help="Override starting balance (default: 10000)"
)
@click.option(
    "--fee-percentage",
    type=float,
...
def backtest(
    ...
    try:
        ...
        initial_balance = (
            ...
            else ab_config.get("initial_balance", 10000.0)
        )
        fee_percentage = (
            fee_percentage
            if fee_percentage is not None
            else ab_config.get("fee_percentage", 0.001)
        )
        slippage_percentage = (
            slippage_percentage
            if slippage_percentage is not None
            else ab_config.get("slippage_percentage", 0.0001)
        )
        commission_per_trade = (
            commission_per_trade
            if commission_per_trade is not None
            else ab_config.get("commission_per_trade", 0.0)
        )
        stop_loss_percentage = (
            stop_loss_percentage
            if stop_loss_percentage is not None
            else ab_config.get("stop_loss_percentage", 0.02)
        )
        take_profit_percentage = (
            take_profit_percentage
        ...
        try:
            ...
            if output_file:
                try:
                    ...
                    console.print(
                        f"[bold green]✓ Backtest trade history saved to {output_file}[/bold green]"
                    )
                except Exception as e:
                    console.print(
                        f"[bold red]Error saving results to {output_file}: {e}[/bold red]"
                    )
            ...
            rejected_trades = [
                t for t in trades_history if t.get("status") == "REJECTED_BY_GATEKEEPER"
            ]
            if rejected_trades:
                console.print(
                    f"[yellow]⚠ Note: {len(rejected_trades)} trade(s) were rejected by RiskGatekeeper[/yellow]\n"
                )
        finally:
...
def portfolio_backtest(
    ctx, asset_pairs, start, end, initial_balance, correlation_threshold, max_positions
):
    """
    ...
    try:
        ...
        if len(asset_pairs) < 2:
            console.print(
                "[bold red]Error: Portfolio backtest requires at least 2 assets[/bold red]"
            )
            raise click.Abort()
        ...
        backtester = PortfolioBacktester(
            asset_pairs=asset_pairs, initial_balance=initial_balance, config=config
        )
        ...
        results = backtester.run_backtest(start_date=start, end_date=end)
    ...
    try:
        ...
        try:
            ...
            train_window_days = int(
                total_days * train_ratio * 0.7
            )  # 70% of train portion for window
            test_window_days = max(
                7, int(total_days * (1 - train_ratio))
            )  # Remaining for test
            ...
            console.print(
                f"[dim]Date range: {start_date} to {end_date} ({total_days} days)[/dim]"
            )
            console.print(
                f"[dim]Windows: train={train_window_days}d, test={test_window_days}d, step={step_days}d[/dim]"
            )
            ...
            if "error" in results:
                console.print(
                    f"[bold red]Walk-Forward Error:[/bold red] {results['error']}"
                )
                console.print(
                    "\n[yellow]Suggestion:[/yellow] Increase the date range or reduce window sizes."
                )
                console.print(
                    f"  Current: {start_date} to {end_date} ({(end_dt - start_dt).days} days)"
                )
                console.print(
                    f"  Required: At least {train_window_days + test_window_days} days"
                )
                raise click.Abort()
            ...
            if engine is not None:
                try:
                    asyncio.run(engine.close())
                    logger.debug("Engine resources closed successfully after walk-forward analysis")
                except Exception as e:
                    logger.error(f"Error closing engine after walk-forward: {e}", exc_info=True)

    except Exception as e:
        console.print(
            f"[bold red]Error running walk-forward analysis:[/bold red] {str(e)}"
        )
...
@click.option("--end-date", required=True, help="End date (YYYY-MM-DD)")
@click.option(
    "--simulations", default=1000, help="Number of simulations (default: 1000)"
)
@click.option("--noise-std", default=0.001, help="Price noise std dev (default: 0.001)")
@click.option("--provider", default="ensemble", help="AI provider to use")
@click.pass_context
def monte_carlo(
    ctx, asset_pair, start_date, end_date, simulations, noise_std, provider
):
    """
    ...
    try:
        ...
        try:
            ...
            if engine is not None:
                try:
                    asyncio.run(engine.close())
                    logger.debug("Engine resources closed successfully after Monte Carlo simulation")
                except Exception as e:
                    logger.error(f"Error closing engine after Monte Carlo: {e}", exc_info=True)

    except Exception as e:
        console.print(
            f"[bold red]Error running Monte Carlo simulation:[/bold red] {str(e)}"
        )
        if ctx.obj.get("verbose"):
            import traceback
