...
def add_api_key(name: str, key: str, description: str):
    ...
    try:
        ...
        if success:
            click.secho(
                f"✅ API key '{name}' added successfully to database",
                fg="green"
            )
            click.secho(
                "💡 Add to config/config.local.yaml for fallback (optional):",
                fg="blue"
            )
            ...
            click.secho(
                f"⚠️  API key '{name}' already exists in database",
                fg="yellow"
            )
            sys.exit(1)
...
def test_api_key(key: str, ip: str):
    ...
    try:
        ...
        if is_valid:
            ...
            click.echo("\n📊 Rate limit info:")
            click.echo(f"  Remaining requests: {metadata.get('remaining_requests', 'N/A')}")
...
def list_api_keys():
    ...
    try:
        with sqlite3.connect(auth_manager.db_path) as conn:
            ...
            click.echo(tabulate(
                table_data,
                headers=["Name", "Key Hash", "Created", "Last Used", "Status", "Description"],
                tablefmt="grid"
            ))
...
def show_audit_log(limit: int, hours: int, failures_only: bool):
    ...
    try:
        with sqlite3.connect(auth_manager.db_path) as conn:
            ...
            if not rows:
                click.secho(f"ℹ️  No audit log entries found in last {hours} hours", fg="cyan")
                return
