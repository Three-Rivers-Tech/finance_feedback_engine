...
class PlatformFactory:
    ...
    class MockPlatform(BaseTradingPlatform):
        ...
        def get_balance(self) -> Dict[str, float]:
            return {"FUTURES_USD": 20000.0, "SPOT_USD": 3000.0, "SPOT_USDC": 2000.0}
    ...
    def create_platform(
        ...
        if platform_name not in cls._platforms:
            ...
            raise ValueError(
                "Unknown platform type. " f"Available platforms: {available}"
            )
        ...
        try:
            ...
            if (
                not getattr(instance, "get_execute_breaker", None)
    ...
    @classmethod
    def register_platform(cls, platform_name: str, platform_class: type) -> None:
        """
        ...
        if not issubclass(platform_class, BaseTradingPlatform):
            raise ValueError(f"{platform_class} must inherit from BaseTradingPlatform")
