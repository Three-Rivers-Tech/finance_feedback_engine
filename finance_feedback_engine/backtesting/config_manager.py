...
class BacktestConfiguration:
    ...
    position_sizing_strategy: str = "fixed_fraction"  # "fixed_fraction", "kelly_criterion", "fixed_amount"
    ...
    visualization_types: List[str] = field(default_factory=lambda: ["equity_curve", "pnl_distribution"])
    ...
    def validate(self) -> List[str]:
        ...
        if self.position_sizing_strategy not in ['fixed_fraction', 'kelly_criterion', 'fixed_amount']:
            errors.append("Position sizing strategy must be 'fixed_fraction', 'kelly_criterion', or 'fixed_amount'")
        ...
        for viz_type in self.visualization_types:
            if viz_type not in valid_visualizations:
                errors.append(f"Invalid visualization type: {viz_type}. Valid types: {valid_visualizations}")
    ...
    def get_all_configurations(self) -> List[BacktestConfiguration]:
        ...
        if not self.parameter_variations:
            ...
            for variation in self.parameter_variations:
                ...
                if errors:
                    logger.warning(f"Configuration validation errors for variation {variation}: {errors}")
...
class BacktestConfigurationManager:
    ...
    def create_scenario(self, name: str, description: str, base_config: BacktestConfiguration) -> BacktestScenario:
    ...
    def run_scenario_comparison(self, scenario_name: str) -> BacktestResultComparison:
        ...
        logger.info(f"Running scenario '{scenario_name}' with {len(configs)} configurations")
