...
class EnhancedRiskAnalyzer:
    ...
    def _calculate_basic_metrics(
        ...
        return {
            ...
            'win_rate': win_rate,
            'avg_win': np.mean([t for t in returns if t > 0]) * 100 if winning_trades > 0 else 0.0,
            'avg_loss': np.mean([t for t in returns if t < 0]) * 100 if losing_trades > 0 else 0.0
        }
    ...
    def _calculate_volatility_metrics(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        avg_rolling_vol = rolling_volatility.mean() if not rolling_volatility.empty else 0.0
    ...
    def _calculate_tail_risk_metrics(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        expected_shortfall_95 = worst_5_percent.mean() if not worst_5_percent.empty else 0.0
        expected_shortfall_99 = worst_1_percent.mean() if not worst_1_percent.empty else 0.0
    ...
    def _calculate_drawdown_metrics(self, equity_curve: List[float]) -> Dict[str, Any]:
        ...
        recovery_factor = abs(total_return / max_drawdown_abs) if max_drawdown_abs != 0 else 0.0
    ...
    def _calculate_risk_adjusted_returns(self, returns: pd.Series) -> Dict[str, Any]:
        ...
        volatility = returns.std()
        if volatility > 0:
            sharpe_ratio = (annualized_return - self.risk_free_rate) / (volatility * np.sqrt(252))
        else:
            sharpe_ratio = 0.0
        ...
        treynor_ratio = (annualized_return - self.risk_free_rate) / 1.0  # Assuming beta=1.0
    ...
    def _calculate_trade_correlation_metrics(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for trade in trades:
            ...
            if trade.get('asset_pair'):
                ...
                elif any(fx in asset_pair for fx in ['USD', 'EUR', 'GBP', 'JPY', 'CHF']):
        ...
        if len(pnl_values) > 1:
            ...
            if len(returns_series) > 1:
                correlation_metrics['pnl_autocorrelation'] = returns_series.autocorr(lag=1)
            else:
        ...
        if asset_types:
            ...
            if total_trades > 0:
                hhi = sum([(count / total_trades) ** 2 for count in asset_counts.values()])
    ...
    def _calculate_concentration_risk(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
        """Calculate concentration risk metrics."""
        if not trades:
            return {}
        
        # Calculate position size concentration
        position_sizes = [abs(trade.get('position_size', 0)) for trade in trades if trade.get('position_size') is not None]
        if position_sizes:
    ...
    def validate_risk_parameters(self, config: Dict[str, Any]) -> List[str]:
        ...
        if not (0.001 <= stop_loss_pct <= 0.5):  # 0.1% to 50%
            errors.append(f"Stop loss percentage ({stop_loss_pct:.3f}) should be between 0.1% and 50%")
        ...
        if not (0.001 <= take_profit_pct <= 1.0):  # 0.1% to 100%
            errors.append(f"Take profit percentage ({take_profit_pct:.3f}) should be between 0.1% and 100%")
        
        # Validate risk per trade
        risk_per_trade = config.get('risk_per_trade', 0.02)
        if not (0.001 <= risk_per_trade <= 0.1):  # 0.1% to 10%
            errors.append(f"Risk per trade ({risk_per_trade:.3f}) should be between 0.1% and 10%")
        
        # Validate maximum daily trades
        max_daily_trades = config.get('max_daily_trades', 20)
        if max_daily_trades <= 0:
            errors.append(f"Maximum daily trades ({max_daily_trades}) should be positive")
        
        # Validate correlation threshold
        correlation_threshold = config.get('correlation_threshold', 0.7)
        if not (0.0 <= correlation_threshold <= 1.0):
            errors.append(f"Correlation threshold ({correlation_threshold:.3f}) should be between 0.0 and 1.0")
