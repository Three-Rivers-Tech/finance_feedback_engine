...
class BacktestPerformanceAnalyzer:
    ...
    def analyze_strategy_performance(
        ...
        analysis['consistency'] = self._analyze_consistency(metrics, trades, equity_curve)
    ...
    def _analyze_consistency(
        ...
        if len(equity_curve) > 60:  # Need at least 60 points for meaningful rolling analysis
            ...
            if len(returns) >= 30:
                ...
                rolling_sharpe = (rolling_returns.mean() / rolling_returns.std()) * np.sqrt(252) if rolling_returns.std().min() > 0 else pd.Series([np.nan] * len(returns))
                
                consistency_metrics['avg_rolling_sharpe'] = rolling_sharpe.dropna().mean()
                consistency_metrics['rolling_sharpe_stdev'] = rolling_sharpe.dropna().std()
        ...
        if len(trades) > 0:
            ...
            if pnl_values:
                ...
                consistency_metrics['pnl_coefficient_of_variation'] = consistency_metrics['pnl_stdev'] / abs(consistency_metrics['pnl_mean']) if consistency_metrics['pnl_mean'] != 0 else float('inf')
        
        return consistency_metrics
    
    def _analyze_risk_return_profile(
        self,
        metrics: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        profile['return_quality'] = self._rate_return_quality(annualized_return_pct, max_drawdown_pct)
    ...
    def _analyze_market_conditions(self, trades: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    def _rate_return_quality(self, annual_return_pct: float, max_drawdown_pct: float) -> str:
    ...
    def _plot_equity_curve(self, equity_curve: List[float]) -> str:
        ...
        ax.plot(equity_series.index, equity_series.values, label='Equity Curve', linewidth=2.0)
        ax.fill_between(equity_series.index, equity_series.values, peak.values, 
    ...
    def _plot_pnl_distribution(self, trades: List[Dict[str, Any]]) -> str:
        ...
        if not pnl_values:
            ...
            ax.text(0.5, 0.5, 'No P&L data available', horizontalalignment='center', 
            ...
            ax.axvline(avg_pnl, color='red', linestyle='--', label=f'Mean: ${avg_pnl:.2f}')
            ax.legend()
    ...
    def _plot_monthly_returns(self, equity_curve: List[float]) -> str:
        ...
        if len(equity_curve) < 30:  # Need at least 30 data points
            ...
            fig, ax = plt.subplots(figsize=(10, 6))
            ax.text(0.5, 0.5, 'Insufficient data for monthly returns', horizontalalignment='center', 
                    verticalalignment='center', transform=ax.transAxes, fontsize=14)
            ...
            monthly_returns = df['returns'].groupby([df.index.year, df.index.month]).sum()
