...
from finance_feedback_engine.backtesting.config_manager import BacktestConfiguration, BacktestConfigurationManager
...
class BacktestOrchestrator:
    ...
    def run_single_backtest(self, configuration: BacktestConfiguration) -> Dict[str, Any]:
        """
    ...
    def optimize_strategy(
        ...
        for combo in combinations:
            ...
            try:
                ...
                logger.debug(f"Config {param_dict}: {metric_to_optimize} = {metric_value:.4f}")
                
            except Exception as e:
                logger.error(f"Error testing config {param_dict}: {e}")
                continue
        
        logger.info(f"Optimization complete. Best {metric_to_optimize}: {best_score:.4f}")
    ...
    def run_walk_forward_analysis(
        ...
        while True:
            ...
            validation_end = validation_start + pd.DateOffset(months=validation_period_months)
            ...
            current_date = training_start + pd.DateOffset(months=validation_period_months)
        ...
        for period in periods:
            logger.info(f"Testing period: {period['training_start']} to {period['validation_end']}")
        ...
        if results['period_results']:
            ...
            results['aggregate_metrics'] = {
                'avg_validation_sharpe': sum(validation_sharpes) / len(validation_sharpes) if validation_sharpes else 0,
                'avg_validation_return': sum(validation_returns) / len(validation_returns) if validation_returns else 0,
                'avg_validation_drawdown': sum(validation_drawdowns) / len(validation_drawdowns) if validation_drawdowns else 0,
                'validation_sharpe_stdev': float('nan') if not validation_sharpes else (sum((x - results['aggregate_metrics']['avg_validation_sharpe'])**2 for x in validation_sharpes) / len(validation_sharpes))**0.5
            }
