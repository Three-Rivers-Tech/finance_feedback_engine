...
class MonteCarloSimulator:
    ...
    def run_monte_carlo(
        ...
        base_final_balance = base_results['metrics'].get('final_balance', backtester.initial_balance)
...
def generate_learning_validation_metrics(
    ...
    if len(outcomes) == 0:
        return {
            'error': f'No outcomes for asset {asset_pair}',
            'total_trades': 0
        }
...
def _calculate_cumulative_regret(outcomes: List, memory_engine) -> Dict[str, Any]:
    ...
    provider_avgs = {
        p: np.mean(pnls) for p, pnls in provider_pnls.items() if pnls
    }
...
def _calculate_concept_drift(outcomes: List) -> Dict[str, Any]:
    ...
    return {
        ...
        'drift_severity': 'HIGH' if drift_score > 0.15 else 'MEDIUM' if drift_score > 0.08 else 'LOW'
    }


def _calculate_thompson_sampling_metrics(outcomes: List, memory_engine) -> Dict[str, Any]:
    """
    ...
    dominant_provider = max(provider_counts, key=provider_counts.get) if provider_counts else None
    exploitation_convergence = recent_provider_counts.get(dominant_provider, 0) / len(recent_window) if recent_window else 0
...
def _calculate_learning_curve(outcomes: List) -> Dict[str, Any]:
    ...
    first_avg_pnl = np.mean([o.realized_pnl for o in first_quartile if o.realized_pnl is not None])

    last_wins = sum(1 for o in last_quartile if o.was_profitable)
    last_win_rate = last_wins / len(last_quartile)
    last_avg_pnl = np.mean([o.realized_pnl for o in last_quartile if o.realized_pnl is not None])

    win_rate_improvement = ((last_win_rate - first_win_rate) / first_win_rate * 100) if first_win_rate > 0 else 0
    pnl_improvement = ((last_avg_pnl - first_avg_pnl) / abs(first_avg_pnl) * 100) if first_avg_pnl != 0 else 0

    return {
        'first_100_trades': {
            'win_rate': first_win_rate,
            'avg_pnl': first_avg_pnl
        },
        'last_100_trades': {
            'win_rate': last_win_rate,
            'avg_pnl': last_avg_pnl
        },
