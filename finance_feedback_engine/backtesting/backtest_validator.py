...
class BacktestValidator:
    ...
    def validate_backtest_results(
        ...
        **kwargs,
        ...
        significance_issues, significance_recs = self._check_statistical_significance(
            metrics, trades
        )
    ...
    def _check_overfitting_indicators(
        ...
        if len(trades) > 0:
            ...
            if len(monthly_pnls) > 0:
                monthly_avg_pnls = [
                    np.mean(pnls) for pnls in monthly_pnls.values() if len(pnls) > 0
                ]
                if len(monthly_avg_pnls) > 1:
                    coefficient_of_variation = np.std(monthly_avg_pnls) / np.mean(
                        monthly_avg_pnls
                    )
            ...
            if len(pnl_values) > 10:
                ...
                if len(pnl_values) > 0:
                    ...
                    if top_20pct_count > 0:
                        ...
                        top_20pct_contribution = (
                            sum(top_20pct_pnls) / sum(pnl_values) * 100
                        )
    ...
    def _perform_walk_forward_analysis(
        self, backtest_results: Dict[str, Any], data_start_date: str, data_end_date: str
    ) -> float:
        """
        ...
        try:
            ...
            train_end_idx = int(
                len(backtest_results.get("backtest_config", {}).get("equity_curve", []))
                * 0.7
            )
            ...
            train_return = (
                (train_equity[-1] - train_equity[0]) / train_equity[0]
                if train_equity[0] != 0
                else 0
            )
            val_return = (
                (val_equity[-1] - val_equity[0]) / val_equity[0]
                if val_equity[0] != 0
                else 0
            )
    ...
    def _perform_sensitivity_analysis(self, backtest_results: Dict[str, Any]) -> float:
        ...
        try:
            ...
            if ulcer_index < 10 and max_drawdown_pct < 20 and 0.5 < sharpe_ratio < 2.0:
                ...
                ulcer_index < 15 and max_drawdown_pct < 30 and 0.3 < sharpe_ratio < 3.0
            ):
    ...
    def _calculate_validation_score(
        ...
        final_score = (
            base_score * sharpe_adjust * dd_adjust * trade_adjust * win_rate_adjust
        )
    ...
    def generate_validation_report(self, validation_result: ValidationResult) -> str:
        ...
        report_lines.extend(["", "KEY VALIDATION METRICS:", "-" * 20])
