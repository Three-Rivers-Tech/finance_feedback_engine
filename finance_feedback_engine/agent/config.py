...
class TradingAgentConfig(BaseModel):
    ...
    sizing_stop_loss_percentage: float = (
        0.02  # Stop loss used for position sizing calculation (2%)
    )
    ...
    def normalize_default_percentages(self):
        ...
        if (
            hasattr(self, "min_confidence_threshold")
            and self.min_confidence_threshold > 1
        ):
    ...
    judged_open_rerank_penalty_ranging_enabled: bool = False
