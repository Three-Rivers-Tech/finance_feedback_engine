"""Provider credential and runtime resolution helpers."""
...
def resolve_runtime_contract(config: Dict[str, Any]) -> RuntimeContract:
    ...
    paper_execution_enabled = bool(
        ...
        or enabled_platforms == frozenset({"paper"})
    )

    agent_cfg = config.get("agent") or {}
    pairs = [str(pair).upper() for pair in (agent_cfg.get("asset_pairs") or [])]
...
def resolve_provider_credentials(config: Dict[str, Any]) -> ProviderCredentials:
    ...
    paper_credentials = config.get("paper") if isinstance(config.get("paper"), dict) else None

    platform_list: Iterable[Any] = config.get("platforms") or []
    if not _is_dict_credentials(coinbase_credentials):
        coinbase_credentials = _find_credentials(platform_list, {"coinbase", "coinbase_advanced"})
    if not _is_dict_credentials(oanda_credentials):
        oanda_credentials = _find_credentials(platform_list, {"oanda"})
    if not _is_dict_credentials(paper_credentials):
        paper_credentials = _find_credentials(platform_list, {"paper", "mock", "sandbox"})

    if resolve_runtime_contract(config).crypto_only_runtime:
...
def _find_credentials(platforms: Iterable[Any], names: set[str]) -> Optional[Dict[str, Any]]:
    for platform_cfg in platforms:
        if not isinstance(platform_cfg, dict):
            continue
        if str(platform_cfg.get("name", "")).lower() not in names:
            continue
