...
class BatchDataIngester:
    ...
    async def ingest_historical_data(
        ...
        try:
            ...
            self.watermark_store.set(asset_pair, timeframe, str(max_timestamp))
    ...
    async def _get_data_provider(self, provider: str):
        ...
        if provider == "alpha_vantage":
            ...
            return AlphaVantageProvider(
                api_key=self.config["alpha_vantage"]["api_key"], config=self.config
            )
            ...
            return CoinbaseDataProvider(credentials=self.config.get("coinbase", {}))
        elif provider == "oanda":
            from finance_feedback_engine.data_providers.oanda_data import (
                OandaDataProvider,
            )

            return OandaDataProvider(credentials=self.config.get("oanda", {}))
        else:
    ...
    def validate_and_clean(
        self, candles: List[Dict[str, Any]], timeframe: str, asset_pair: str
    ...
    async def _write_to_delta(
        self, df: pd.DataFrame, table_name: str, asset_pair: str, timeframe: str
    ):
    ...
    def save_dead_letter_queue(
        self, records: List[Dict[str, Any]], error: str = "unknown"
    ):
        ...
        for asset_pair in asset_pairs:
            for timeframe in timeframes:
                try:
                    ...
                    results.append(
                        {"asset_pair": asset_pair, "timeframe": timeframe, **result}
                    )
                    ...
                    logger.error(f"Failed to ingest {asset_pair} {timeframe}: {e}")
                    results.append(
                        {
                            "asset_pair": asset_pair,
                            "timeframe": timeframe,
