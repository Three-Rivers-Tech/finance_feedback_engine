...
class TwoPhaseAggregator:
    ...
    async def aggregate_two_phase(
        ...
        confidence_threshold = self.two_phase_config.get('phase1_confidence_threshold', 0.75)
        agreement_threshold = self.two_phase_config.get('phase1_agreement_threshold', 0.6)
        require_premium_for_high_stakes = self.two_phase_config.get('require_premium_for_high_stakes', True)
