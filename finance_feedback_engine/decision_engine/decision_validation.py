...
def validate_decision_comprehensive(
    ...
    if action not in ['BUY', 'SELL', 'HOLD']:
        errors.append(
            f"Invalid action '{action}'. Must be BUY, SELL, or HOLD"
        )
    ...
    try:
        ...
        if not 0 <= conf <= 100:
            errors.append(
                f"Confidence {conf} out of range [0, 100]"
            )
    ...
    if 'stop_loss_percentage' in decision:
        try:
            ...
            if not 0 < stop_loss <= 100:
                errors.append(
                    f"Stop loss {stop_loss}% out of range (0, 100]"
                )
        except (TypeError, ValueError) as e:
