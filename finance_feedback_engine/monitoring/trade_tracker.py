...
class TradeTrackerThread(threading.Thread):
    ...
    def run(self):
        ...
        try:
            while not self._stop_event.is_set():
                ...
                if not still_open:
                    logger.info(
                        f"Position closed detected: {self.trade_id}"
                    )
            ...
            if self._stop_event.is_set():
                logger.info(
                    f"Trade tracker stopped externally: {self.trade_id}"
                )
                self._finalize_trade(forced_stop=True)
                
        except Exception as e:
            logger.error(
                f"Error in trade tracker {self.trade_id}: {e}",
                exc_info=True
            )
        finally:
            self._running = False
    ...
    def _update_position_status(self) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error updating position {self.trade_id}: {e}",
                exc_info=True
            )
    ...
    def _finalize_trade(self, forced_stop: bool = False):
        ...
        try:
            ...
            logger.error(
                f"Error in metrics callback for {self.trade_id}: {e}",
                exc_info=True
            )
    ...
    def stop(self, timeout: float = 5.0) -> bool:
        ...
        if self.is_alive():
            logger.warning(
                f"Trade tracker {self.trade_id} did not stop within timeout"
            )
            return False
