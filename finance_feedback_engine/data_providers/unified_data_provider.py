...
class UnifiedDataProvider:
    ...
    def discover_available_pairs(
        ...
        Queries Coinbase and/or Oanda APIs to get full list of available pairs,
        then standardizes them to a common format (e.g., 'BTCUSD', 'EURUSD').

        Args:
            exchanges: List of exchanges to query. Options: ['coinbase', 'oanda']
                      If None, queries all configured exchanges.

        Returns:
            List of standardized asset pairs (e.g., ['BTCUSD', 'ETHUSD', 'EURUSD'])
        ...
        if "coinbase" in exchanges and self.coinbase:
            try:
                ...
                logger.info(f"Discovered {len(products)} pairs from Coinbase")
        ...
        if "oanda" in exchanges and self.oanda:
            try:
                ...
                logger.info(f"Discovered {len(instruments)} pairs from Oanda")

            except Exception as e:
                logger.warning(f"Oanda discovery failed: {e}")

        # Deduplicate and sort
        unique_pairs = sorted(list(set(discovered)))

        logger.info(
            f"Discovery complete: {len(unique_pairs)} unique pairs "
            f"from {len(exchanges)} exchange(s)"
        )

        return unique_pairs

    def _standardize_pair(self, raw_pair: str) -> str:
        """
        Standardize pair format across exchanges.

        Converts various formats to uppercase, no separators:
        - BTC-USD → BTCUSD
        - EUR_USD → EURUSD
        - btc/usd → BTCUSD

        Args:
            raw_pair: Raw pair string from exchange
