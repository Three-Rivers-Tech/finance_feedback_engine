...
class TimeframeAggregator:
    ...
    def _calculate_sma(
        self, candles: List[Dict[str, Any]], period: int
    ) -> Optional[float]:
    ...
    def _calculate_macd(
        ...
        macd_result = ta.macd(
            closes, fast=fast_period, slow=slow_period, signal=signal_period
        )
        ...
        return {
            "macd": round(
                float(last_row[f"MACD_{fast_period}_{slow_period}_{signal_period}"]), 4
            ),
            "signal": round(
                float(last_row[f"MACDs_{fast_period}_{slow_period}_{signal_period}"]), 4
            ),
            "histogram": round(
                float(last_row[f"MACDh_{fast_period}_{slow_period}_{signal_period}"]), 4
            ),
        }

    def _calculate_stochastic_oscillator(
        self, candles: List[Dict[str, Any]], k_period: int = 14, d_period: int = 3
    ) -> Optional[Dict[str, float]]:
        """
        ...
        stoch_result = ta.stoch(
            df["high"], df["low"], df["close"], k=k_period, d=d_period, smooth_k=1
        )
    ...
    def _calculate_cci(
        self, candles: List[Dict[str, Any]], period: int = 20
    ) -> Optional[float]:
        """
    ...
    def _calculate_ichimoku_cloud(
        ...
        return {
            ...
            "tenkan_sen": (
                round(tenkan_current, 2) if not pd.isna(tenkan_current) else None
            ),
            "kijun_sen": (
                round(kijun_current, 2) if not pd.isna(kijun_current) else None
            ),
            "senkou_a": (
                round(senkou_a_current, 2) if not pd.isna(senkou_a_current) else None
            ),
            "senkou_b": (
                round(senkou_b_current, 2) if not pd.isna(senkou_b_current) else None
            ),
            "chikou_span": (
                round(chikou_current, 2) if not pd.isna(chikou_current) else None
            ),
    ...
    def _calculate_bollinger_bands(
        self, candles: List[Dict[str, Any]], period: int = 20, std_dev: float = 2.0
    ) -> Optional[Dict[str, float]]:
        """
    ...
    def _calculate_signal_strength(self, indicators: Dict[str, Any]) -> int:
        """
    ...
    def _analyze_trend_alignment(
        self, timeframe_analysis: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        weights = {"1d": 40, "4h": 25, "1h": 15, "15m": 10, "5m": 5, "1m": 5}
        ...
        if total_weight == 0:
            return {"direction": "unknown", "confluence_strength": 0, "conflicts": []}
    ...
    def _generate_entry_signals(
        ...
        signals = {"buy_signals": [], "sell_signals": [], "overall": "neutral"}
