...
class MockLiveProvider:
    ...
    def advance_pulse(self) -> bool:
        ...
        if next_index >= self.total_candles:
            logger.debug(f"End of data reached: index {next_index} >= {self.total_candles}")
            return False

        # Advance to next pulse
        self.current_index = next_index
        self.pulse_index += 1
        logger.debug(f"Pulse {self.pulse_index}: advanced to candle index {self.current_index}")
        return True
    ...
    def _generate_timeframe_candles(
        ...
        try:
            ...
            if has_lowercase:
                ...
                logger.warning(f"Could not find OHLCV columns for {minutes_per_candle}m aggregation")
                return []

            # Resample and aggregate
            resampled = df[list(agg_spec.keys())].resample(f'{minutes_per_candle}min').agg(agg_spec)
            ...
            for date, row in resampled.tail(max_history).iterrows():
                try:
                    ...
                    if pd.isna(open_val) and pd.isna(high_val) and pd.isna(low_val) and pd.isna(close_val):
                        continue
                    ...
                    logger.debug(f"Skipping {minutes_per_candle}m row due to error: {row_err}")
                    continue
    ...
    def _simple_aggregation(self, df: pd.DataFrame, minutes_per_candle: int) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                'volume': 0,
                'date': str(df.index[-1]) if hasattr(df.index, '__getitem__') else 'unknown',
            }
        except Exception as e:
            logger.debug(f"Simple aggregation failed: {e}")
            return {'open': 0, 'high': 0, 'low': 0, 'close': 0, 'volume': 0, 'date': 'unknown'}
