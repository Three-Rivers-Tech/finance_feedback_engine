...
async def update_config(
    ...
    try:
        ...
        if request.stop_loss_pct is not None:
            engine.config['agent']['sizing_stop_loss_percentage'] = request.stop_loss_pct
            updates['stop_loss_pct'] = request.stop_loss_pct

        if request.position_size_pct is not None:
            engine.config['agent']['sizing_risk_percentage'] = request.position_size_pct
            updates['position_size_pct'] = request.position_size_pct

        if request.confidence_threshold is not None:
            engine.config['agent']['min_confidence_threshold'] = request.confidence_threshold
            updates['confidence_threshold'] = request.confidence_threshold

        if request.max_concurrent_trades is not None:
            engine.config['agent']['max_concurrent_trades'] = request.max_concurrent_trades
            updates['max_concurrent_trades'] = request.max_concurrent_trades

        if request.provider_weights is not None:
            engine.config['ensemble']['provider_weights'] = request.provider_weights
            updates['provider_weights'] = request.provider_weights
